...
void recountMatrix(pnode t) {
    if (t) {
        if(t->rchild && t->rchild->rev) pushDownRev(t->rchild);
        if(t->lchild && t->lchild->rev) pushDownRev(t->lchild);
...
mtx joinMatrices(mtx m1, mtx m2){
    ULL a = m1.gg, b = m1.gr, c = m1.rg, d = m1.rr;
    ULL A = m2.gg, B = m2.gr, C = m2.rg, D = m2.rr;

    ULL GG = (a + A + a * C + b * C + b * A) % mod;
    ULL GR = (b + B + a * D + b * B + b * D) % mod;
    ULL RG = (c + C + c * C + d * A + d * C) % mod;
    ULL RR = (d + D + c * D + d * D + d * B) % mod;
...
void recountMatrix(pnode t){
    ...
    if(t) {
        if(t->rev) pushDownRev(t);

        t->m = {{0, 0},
                {0, 0}};
        t->id == 'R' ? t->m.rr = 1 : t->m.gg = 1;

        if (t->lchild) {
            if(t->lchild->rev) pushDownRev(t->lchild);
            t->m = joinMatrices(t->lchild->m, t->m);
        }

        if (t->rchild) {
            if(t->rchild->rev) pushDownRev(t->rchild);
            t->m = joinMatrices(t->m, t->rchild->m);
        }
    }
}
